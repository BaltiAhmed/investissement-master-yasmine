  ...
  const onRefresh = useCallback(() => {
    ...
    const sendRequest = async () => {
      const response = await fetch(
        `http://192.168.1.185:5000/api/production/projet/${EId}`
      );
...
ListeProduction.navigationOptions = (navData) => {
  return {
    headerTitle: "Liste Production",
  };
};

export default ListeProduction;
